      ...
      {analysis.action_steps && analysis.action_steps.length > 0 && (
        ...
        >
          ...
          <div className="space-y-2.5">
            {analysis.action_steps.map((step: string, idx: number) => (
              <motion.div
                key={idx}
                initial={{ opacity: 0, x: -8 }}
                animate={{ opacity: 1, x: 0 }}
              ...
              >
                <div className="w-5 h-5 rounded-full bg-emerald-200 flex items-center justify-center flex-shrink-0 mt-0.5 text-xs font-bold text-emerald-700">
                  {idx + 1}
                </div>
